...
export default function C3ModuleDetail() {
  ...
  const moduleConfigs: Record<string, {
    ...
    "stamp-creator": {
      name: "スタンプ作成システム",
      description: "AI絵文字提案・マルチアセット構成・Telegramステッカー出力",
      icon: FileText,
      apiEndpoint: "/api/stamps/create",
      features: [
        "AI絵文字提案",
        "マルチアセット構成",
        "リアルタイムプレビュー",
      ...
      actions: [
        { label: "スタンプ作成", path: "/api/stamps/create", testId: "button-create-stamp" }
      ]
    }
  };
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <header className="relative px-8 py-6 border-b border-[white]/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-[white] text-xl font-bold tracking-[0.2em]">{config.name}</h1>
                <p className="text-[white]/60 text-xs tracking-[0.3em]">MODULE DETAILS</p>
              </div>
            </div>
        ...
        <main className="flex-1 p-8">
          <div className="max-w-7xl mx-auto">
            <div className="mb-8 border border-[white]/30 p-6"
            ...
            <div className="grid lg:grid-cols-2 gap-8 mb-8">
              <div>
                <div className="inline-block mb-4 px-4 py-1 border border-[white]/50">
                  <h3 className="text-[white] text-xs tracking-[0.4em]">FEATURES</h3>
                </div>
                <div className="space-y-3">
                  {config.features.map((feature, index) => (
                    <div key={index} className="flex items-start gap-3 text-white/80">
                      <div className="mt-1 w-2 h-2 bg-[white]" style={{ clipPath: 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)' }} />
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <div className="inline-block mb-4 px-4 py-1 border border-[white]/50">
                  <h3 className="text-[white] text-xs tracking-[0.4em]">ACTIONS</h3>
                </div>
                <div className="space-y-3">
                  {config.actions.map((action, index) => (
                    <button
                      ...
                      className="w-full group px-4 py-3 border border-[white]/50 hover:border-[white] hover:bg-[white]/10 text-left transition-all flex items-center justify-between"
            ...
            {stats && (
              <div className="border border-[white]/30 p-6"
                ...
                <div className="flex items-center gap-2 mb-4">
                  <div className="text-[white] text-sm tracking-[0.3em]">STATISTICS</div>
                  <div className="flex-1 h-px bg-[white]/30" />
                </div>
                <pre className="text-[white] text-sm overflow-auto" data-testid="stats-display">
