...
class FileProcessor:
    ...
    @staticmethod
    def process_image_bytes(file_bytes: bytes, max_width: int = 1920) -> bytes:
        """Process raw image bytes: EXIF, resize if needed, save as WebP. Fail-safe returns original."""
        if not file_bytes:
            return b""
        try:
    ...
    async def process_image(self, request: ImageProcessingRequest) -> ProcessingResult:
        """Process image with specified operations. Use image_base64 when provided."""
        start_time = datetime.utcnow()
        try:
            ...
            if not file_bytes:
                return ProcessingResult(
                    ...
                    error="Provide image_base64 or load asset data",
                )
            ...
            fmt = (request.output_format and request.output_format.value.upper()) or "WEBP"
            quality = request.quality or 85
            if fmt == "WEBP":
                img.save(output_buffer, format="WEBP", quality=quality, method=6)
            else:
                img.save(output_buffer, format=fmt or "JPEG", quality=quality, optimize=True)
            out_bytes = output_buffer.getvalue()
            processing_time = (datetime.utcnow() - start_time).total_seconds() * 1000
            reduction = (1.0 - len(out_bytes) / original_size) * 100.0 if original_size else 0.0
            logger.info("Image processed", asset_id=request.asset_id, processing_time_ms=processing_time)
            return ProcessingResult(
                success=True,
                processed_asset_id=str(uuid4()),
                processing_time_ms=processing_time,
                file_size_reduction=round(reduction, 1),
                metadata={"operations_applied": len(request.operations)},
            )
        except Exception as e:
            processing_time = (datetime.utcnow() - start_time).total_seconds() * 1000
            logger.error("Image processing failed", asset_id=request.asset_id, error=str(e))
            return ProcessingResult(success=False, processing_time_ms=processing_time, error=str(e))
