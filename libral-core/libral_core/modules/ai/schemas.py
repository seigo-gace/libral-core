...
# Local LLM inference (Sovereign Autarchy) - 高速稼働・DoS 対策
INFER_LOCAL_PROMPT_MAX = 32 * 1024
INFER_LOCAL_SYSTEM_MAX = 4 * 1024


class InferLocalRequest(BaseModel):
    """Request for local LLM inference (Node OSS Manager bridge)."""
    model_id: Optional[str] = Field(None, max_length=128)
    prompt: str = Field(..., min_length=1, max_length=INFER_LOCAL_PROMPT_MAX)
    system_prompt: str = Field(default="You are a helpful local assistant.", max_length=INFER_LOCAL_SYSTEM_MAX)


class InferLocalResponse(BaseModel):
