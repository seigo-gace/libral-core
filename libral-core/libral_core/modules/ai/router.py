...
from .schemas import (
    ...
    EvaluationRequest, EvaluationResponse, InferLocalRequest, InferLocalResponse,
    QueryCategory
)
...
def _verify_internal_secret(x_internal_secret: Optional[str] = Header(None)) -> bool:
    """本番で infer_local を内部のみに制限する場合: LIBRAL_INTERNAL_SECRET を設定し、Node から同じ値を X-Internal-Secret で送る。未設定なら常に True。"""
    secret = os.getenv("LIBRAL_INTERNAL_SECRET")
    if not secret:
        return True
    return bool(x_internal_secret and x_internal_secret == secret)


# Local LLM inference (Sovereign Autarchy - Node OSS Manager calls this)
@router.post("/infer_local", response_model=InferLocalResponse)
async def infer_local(
    req: InferLocalRequest,
    x_internal_secret: Optional[str] = Header(None, alias="X-Internal-Secret"),
):
    """Run inference on local OSS model (Worker). Internal Node bridge only when LIBRAL_INTERNAL_SECRET is set."""
    if not _verify_internal_secret(x_internal_secret):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
    try:
        worker = LocalLLMWorker()
        text = await worker.infer(
            user_prompt=req.prompt,
            system_prompt=req.system_prompt,
        )
        return InferLocalResponse(text=text)
    except Exception as e:
        logger.error("Local infer error", error=str(e))
        return InferLocalResponse(text=f"[local-infer:error] {e!s}")
